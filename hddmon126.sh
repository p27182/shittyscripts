...
echo 'Querying HDD temps for /dev/md126...'
...
do
	...
	hddtemps=$(sudo hddtemp /dev/sde /dev/sdh /dev/sdf /dev/sdg /dev/sdd)
	...
	do
		...
		curl -s -XPOST 'http://10.0.0.116:8086/write?db=ananke' --data-binary "HDDtemps126,name=$dev.$brand.$model value=$temp"
